...
class EuclideanTest {
    ...
    @Test
    public void check_euclideanDistance() {
        p1 = List.of(2.0, 3.0);
        p2 = List.of(5.0, 7.0);
        Double actual_value=Distance.get(p1, p2);
        assertEquals(5,actual_value);

    }

    @Test
    public void check_eMatrix(){
        p1 = List.of(2.0, 3.0);
        p2 = List.of(5.0, 7.0);
        List<List<Double>> points = List.of(p1,p2);
        List<List<Double>> act=Distance.matrix(points);
        List<List<Double>> exp=new ArrayList<>();
        exp.add(Arrays.asList(0.0,5.0));
        exp.add(Arrays.asList(5.0,0.0));
